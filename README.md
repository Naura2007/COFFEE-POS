...
## Rancangan Kode
### AdminGUI
Kelas AdminGUI adalah komponen Graphical User Interface (GUI) utama dalam arsitektur aplikasi Point of Sale (POS) Kopi. Kelas ini berfungsi sebagai Panel Administrasi yang memungkinkan manajer atau administrator melakukan manajemen data produk. Peran Utama: Kelas ini mengimplementasikan prinsip CRUD (Create, Read, Update, Delete) terhadap dua entitas data utama: Menu (MenuItemModel) dan Topping (Topping). Kelas ini menjembatani antara tampilan (GUI) dan lapisan layanan/persisten (MenuService, ToppingService, JsonDB).
  1. public AdminGUI()
